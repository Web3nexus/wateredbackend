...
class EventRevenueChart extends ChartWidget
{
    ...
    public static function canView(): bool
    {
        return auth('admin')->user()?->hasPermissionTo('access_financials', 'admin')
            && auth('admin')->user()?->hasPermissionTo('access_events', 'admin');
    }

    protected function getData(): array
    {
        $data = Trend::query(
            EventRegistration::whereIn('payment_status', ['completed', 'paid', 'success', 'confirmed', 'booked', 'successful'])
                ->orWhereIn('status', ['confirmed', 'paid', 'successful'])
        )
            ->between(
